...
- **v0.25.5**（2026-09-04 revert 回滚，原版 2026-08-29）：应用户要求 revert v0.25.6/7（061dfbe/00583ab），回到"墙层零溢出单格写自身值（v0.25.4）+ 记忆区统一 dimA（v0.25.5）"状态。重建部署冒烟 err=0。**遗留已知问题：current 墙中阴影位置/渐变与原版有差异（未解决）**。
- 墙渲染迭代教训（v0.25.2→v0.25.7 六连败，2026-09-04 回滚）：像素观感问题必须先在离线模拟里对齐"原版同场景截图"基准再上真机，每次只改一个变量，改完同场景三图对比；v0.25.6/7 的"墙面受光传播"思路见 git 历史（7b7525c/5098bc0），方向待验证、实现两处细节均错。基准截图：knowledge/experiments/shot_vanilla|current|classic.png。
