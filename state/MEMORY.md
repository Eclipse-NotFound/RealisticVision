...
- **v0.25.4**（2026-08-29）：classic 墙层改**零溢出单格写入**——只写 (2x+1,2y+1) 一格（位移下半格恰好精确覆盖整瓦片），根除 4 格写法的 W/N 20px 溢出（v0.25.2 边缘亮带/v0.25.3 脏迹的共同根因）；雾层墙像素全协调值（回退 v0.25.3 例外）。离线像素模拟四版对比确认（rv0253-wall-smudge-sim2.py）。冒烟 err=0。**待用户目检**。
- v0.25.0-3（08-27）：渲染模式注入选项页（v0.25.0）；墙对齐原版系列迭代（v0.24.9 统一光照 → v0.25.1 四分格被否 → v0.25.2 位移单格"大改善"但有边缘亮带 → v0.25.3 边界例外更脏）。
