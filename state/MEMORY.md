...
- **v0.25.3**（2026-08-27）：修复屏幕下/左边缘 20px 亮带——房间边界墙（四边）的雾层墙像素改用自身 aArr（原用内侧地板协调值，边界墙 SE 半格被照亮成亮带）。冒烟 err=0。**待用户目检四边**。
- v0.25.2（同日）：墙层写自身公式值单值 + 整层位移半格（值块落原版墙像素屏上位置，SE 半格露协调值），阴影线在瓦片中线、保持 v0.25.0 单值干净观感——用户确认"大改善"。
- v0.25.1（同日，被否）：四分格象限采样——20px 拼块发脏。
- v0.25.0（同日）：渲染模式切换注入哔哔小马选项页原生列表。**注入行待用户打开选项页目检**。
- v0.24.9→v0.24.8（同日）：墙并入雾层同公式（统一光照，无恒黑）；classic 30Hz 门控失效修复（frameCount 双递增，D21）+ autotest 埋点——已 8 轮真机自动化验证。
...
1. **v0.25.3 屏幕边缘**：下/左（及上/右）边缘亮带消失，边界墙整面明暗与自身一致；
2. **v0.25.2 墙观感**：墙内阴影线在瓦片中线侧、单值干净、地板侧无稀释亮带；
3. **v0.25.0 注入行**：哔哔小马 → 选项页列表末行「视野渲染模式」点击循环并持久化；
4. **v0.24.8 classic 流畅度**（30Hz 修复后首次真 30Hz）；
5. v0.24.7 原清单沿用：亮边与厚墙内部、念力宽限、部分可见（hpbar/武器同步裁剪）、门、暗区武器/手、营地无雾、F10/F11/config、帧率手感、与 Sandevistan/RConnect 兼容观感；
6. 任意会话按一次 F11/F12 → 查 RVision.log "auto config write error" 定位写盘失败；
7. 向 RConnect 维护者报告 autoTravelLand 到达即死问题。
