...
- **v0.25.7**（2026-08-29）：修 v0.25.6 两处错误——classic 墙层改回自身公式值（协调值让墙面蒙邻地板记忆灰；原版墙面=墙自身 visi 场错位马赛克）、current 受光墙 br=亮值+TL 象限强制黑（br=0 导致上排墙整墙暗，"上暗下亮"语义分两层：br 管传递/litArr==2 管自身压暗）。双模式冒烟 err=0。**待用户三图复检**。
- v0.25.4-6（08-29）：墙层零溢出单格（4）、记忆区统一 dimA（5）、墙面受光传播（6，值语义在 7 修正）。
