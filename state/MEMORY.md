...
- 2026-09-20用户提出“正在考虑通过类似classic模式的模糊效果实现更好的抗锯齿”。这是设计讨论，尚未拍板渐变宽度或授权合入；优先评估保留current投影轮廓、借用classic连续亮度过渡的分层柔化原型。墙内黑芯保护不等于禁止地板边界的有意渐变。
...
2. 抗锯齿先讨论用户提出的classic式柔化：保留current几何、地板做连续渐变、墙独立保护、柔化不写探索记忆；宽度与边缘外轻微变亮尚未定案。上轮无钳制实验未满足严格黑区条件，不等于否定有意软边。连续轮廓覆盖率/边缘距离为后备方向；仍需验门水破墙、换房、缩放、探索与单位裁剪。
