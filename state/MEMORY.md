...
- **v0.25.5**（2026-08-29）：classic 记忆区统一记忆暗色（记忆目标 `explored ? dimA : 255`，撤销 `max(gameA,dimA)` 对弱照明瓦片暗斑的保留）——对齐 current 的 fillMemoryTile 语义，修记忆区斑驳脏迹（用户三截图对比定位）。冒烟 err=0。**待用户目检**。
- v0.25.4（08-29）：墙层零溢出单格写入（只写 (2x+1,2y+1)，位移下恰覆盖整瓦片），根除 4 格写法 W/N 20px 溢出（边缘亮带/脏迹根因）；离线像素模拟验证。
