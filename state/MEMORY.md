...
- **v0.25.0**（2026-08-27）：渲染模式切换注入哔哔小马选项页原生列表（克隆原生行点击循环，与 F12 共用 cycleMode；失败静默降级到左下面板）。**加载冒烟通过；注入行待用户打开选项页目检**。
- v0.24.9（同日）：classic 墙内部对齐原版——墙并入雾层同公式（游戏 visi + memCur 记忆混合），恒黑墙层（v0.23-v0.24.1）退役。冒烟通过（err=0、30Hz 无回归、classic 帧耗时再降 ~25%），待用户游戏内确认墙观感。
...
1. **v0.25.0 注入行**：哔哔小马 → 选项页列表末行出现「视野渲染模式」，点击循环三种模式并持久化；其他选项子卡不显示本行；观感/位置异常则 revert；
2. **v0.24.9 墙观感**：classic 亮区墙显示纹理、记忆区墙渐暗、厚墙/未探索墙保持黑；切换 current↔classic 无残留；
