...
- **v0.25.2**（2026-08-27）：classic 墙渲染定型——墙层写自身公式值**单值**+整层位移半格（值块落原版墙像素屏上位置，SE 半格露雾层协调值），阴影线在瓦片中线（对齐原版）且保持 v0.25.0 单值干净观感。冒烟 err=0。**待用户目检**。
- v0.25.1（同日，被否）：四分格象限采样修复偏移但产生 20px 拼块（用户实测"脏"）。
- v0.25.0（同日）：渲染模式切换注入哔哔小马选项页原生列表（克隆原生行点击循环，与 F12 共用 cycleMode；失败静默降级到左下面板）。**注入行待用户打开选项页目检**。
...
1. **v0.25.2 墙观感**：classic 墙内阴影线在瓦片中线侧（原版位置）、单值干净无拼块、地板侧无稀释亮带、墙排上/左 20px 溢出不显眼；
2. **v0.25.0 注入行**：哔哔小马 → 选项页列表末行「视野渲染模式」点击循环并持久化；其他选项子卡不显示本行；
