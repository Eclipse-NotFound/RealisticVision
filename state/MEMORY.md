...
- **v0.25.1**（2026-08-27）：修复 v0.24.9 引入的 classic 墙内阴影偏移——墙层复活为**四分格公式值**（TL=自身/TR=东/BL=南/BR=东南 的 aArr 值 = 原版错位马赛克的瓦片对齐呈现），雾层墙像素回插邻域协调值防地板侧稀释；墙公式值仍来自统一光照（无恒黑）。冒烟 err=0。**待用户目检墙内阴影位置**。
- v0.25.0（同日）：渲染模式切换注入哔哔小马选项页原生列表（克隆原生行点击循环，与 F12 共用 cycleMode；失败静默降级到左下面板）。**加载冒烟通过；注入行待用户打开选项页目检**。
- v0.24.9（同日）：classic 墙内部对齐原版第一步（墙并入雾层同公式、恒黑墙层退役）——其"墙显示自身单一值"被 v0.25.1 修正为四分格。
...
1. **v0.25.1 墙内阴影位置**：classic 亮墙全面亮、暗墙影子压在瓦片中线侧（原版错位马赛克观感）、地板侧无稀释亮带；
