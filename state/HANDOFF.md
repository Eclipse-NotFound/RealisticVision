...
- 版本 **v0.21**，`release/RealisticVisionMod.swf` 已部署（Loader 子域注入，
...
- **验证未完成（v0.21 未实测）**：① 炮塔类敌人在明暗边界部分可见（无二分）
  ② 敌人边界无颗粒感（位图掩膜圆滑）③ 墙炮塔不消失 ④ 位图掩膜无白块/
  闪烁（生命周期回归项）。机制已离线验证，待游戏内实测。
