...
- 版本 **v0.19**，`release/RealisticVisionMod.swf` 已部署（Loader 子域注入，
...
- 三档渲染模式（`config.txt` 的 `mode`，F12 轮换，PipBuck 选项页面板 + F10
  ...
  - `classic`（仿原版）：**v6 = 游戏值优先**（1px/瓦片雾层 + smoothing=true
    双线性 + 半格错位 x=-20/y=-60 写 y+1 行 + 每帧读游戏 tile.visi，结构
    复刻原版 lightBmp）。规则：`a=(1-visi)×255` 全程游戏值原样（衰减环
    lDist1=300→lDist2=1000 梯度/淡入 +0.1 帧/lighting2 呼吸/retDark -0.025
    消退/光源物/门景/墙恒黑+错位渗亮）；**唯一干预**：非 retDark && 非墙
    && 视野外 && a<dimA → a=dimA（记忆区下限——内部均匀 166，边界保留游戏
    衰减梯度 166→黑渐变）。**不变量：模组只暗化不亮化**（mod≥game 处处
    成立，"该为阴影处渲染为亮部"不可能）；暗化仅限记忆区。门景 doorBoost
    写 visi 生效；每帧变化才写像素（lastA 缓存，站立零写入）；
...
- **验证未完成（v0.19 classic v6 未实测）**：① 记忆区边界 = 游戏衰减梯度
  （166→黑渐变，无亮步/无块状）② 墙恒黑+错位渗亮（无 166 亮块）③ retDark
  房间纯原版消退 ④ 视野内衰减环梯度（远端阴影是原版行为）⑤ 帧率。规则
  不变量已离线验证（2400 组合只暗化不亮化、边界单调），待游戏内实测。
