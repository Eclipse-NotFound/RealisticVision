...
- 版本 **v0.20.1**，`release/RealisticVisionMod.swf` 已部署（Loader 子域注入，
...
- **验证未完成（v0.20.1 未实测）**：① 墙恒黑（无墙边亮带，含 current→
  classic 切换后）② 记忆边界移动动画平滑 ③ 画面右/下无亮边缺口 ④ retDark
  消退 ⑤ 帧率。机制已离线验证，待游戏内实测。
