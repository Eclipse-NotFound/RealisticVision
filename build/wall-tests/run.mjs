...
const harness=edges?'WallEdgeHarness':'WallHarness';
...
const out=path.join(mod,'build/wall-test-output',edges?'edges':'');
...
fs.writeFileSync(path.join(out,'app-wall-test.xml'),`<?xml version="1.0"?><application xmlns="http://ns.adobe.com/air/application/30.0"><id>rv-wall-fixture${edges?'-edges':''}</id><versionNumber>1.0</versionNumber><filename>${harness}</filename><initialWindow><content>${harness}.swf</content><visible>false</visible><renderMode>cpu</renderMode></initialWindow></application>`);
...
process.exit(run.status===0 && log.includes(edges?'WALL_EDGE_TESTS PASS':'WALL_TESTS PASS')?0:1);
