...
if(seams && !training)throw Error('--seams requires --training');
const scenario=training?(seams?'training-seams':'training'):'random_mane';
...
fs.writeFileSync(path.join(boot,'RealisticVisionMod.as'),`package { import flash.display.Sprite; public class RealisticVisionMod extends Sprite { public static var probe:GameProbe; public static function init(main:*):void { probe=new GameProbe(main,"${scenario}"); } } }`);
...
const completed=seams?'GAME_PROBE CAPTURED training seams (visual diagnosis only)':training?'GAME_PROBE CAPTURED training (visual diagnosis only)':'GAME_PROBE PASS';
process.exit(r.status===0 && log.includes(completed)?0:1);
