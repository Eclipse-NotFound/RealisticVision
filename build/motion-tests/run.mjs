...
const checks=process.argv.includes('--checks'),harness=checks?'SoftHarness':'MotionHarness';
if(!/^[a-z0-9-]+$/.test(variant))throw Error('Invalid variant');
const out=path.join(mod,'build/motion-test-output',variant+(checks?'-checks':''));fs.mkdirSync(out,{recursive:true});
